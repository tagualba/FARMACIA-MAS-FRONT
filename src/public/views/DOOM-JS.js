...
function buildFilters(categorys, marcas)
{
    ...
    {   
        var resSubCategorys="";
        var auxCategory = categoryHtmlBase.replace("@IDCATEGORIA", category.id);
        auxCategory = auxCategory.replace("@CATEGORIANOMBRE", category.name);
        ...
        for(let subCategory of category.subCategorys)
        {
            ...
            filter.subCategorys.push("SubCategoriaCheckBox" + subCategory.id);
        }

        resCategorys += auxCategory.replace("@SUBCATEGORIASCONTAINER", resCategorys);
    }

    doomInner(resCategorys);
    return filter;
}